...
- 사용자가 "아까 한 질문", "이전 질문", "방금 물어본 것" 등 과거 대화 내용을 참조하는 경우: 이전 대화 이력은 저장되지 않으므로 **"이전 대화 내역은 확인이 어렵습니다. 궁금하신 내용을 다시 말씀해 주세요." 한 문장**으로만 답하세요. 반복적인 사과·예시 나열·대안 제시를 절대 덧붙이지 마세요.
${welcomeContext ? `\n[바로 직전 인사말]\n당신이 사용자에게 먼저 건넨 말:\n${welcomeContext}` : ""}
${context ? `\n[데이터]\n${context}` : "\n[데이터 없음]"}`;
